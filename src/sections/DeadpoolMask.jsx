...
import DeadpoolHead from "../components/DeadpoolHead.jsx";
...
const DeadpoolMask = () => {
  ...
  const handleMove = (clientX, clientY) => {
    if (containerRef.current && hoverCanvasRef.current) {
      ...
      const moveFactor = isMobile ? 3 : 5; // Reduced from 10 to make it smoother
  ...
  useEffect(() => {
    ...
    if (solid) {
      if (showMask) {
        ...
        solid.style.WebkitMaskSize = size;
        solid.style.maskSize = size;
        solid.style.transition = "opacity 0.1s ease-out";
  ...
  const finalMaskScale = isMobile ? 1.0 : 1.7;
  const finalMaskPosition = isMobile ? [-0.2, -0.3, 0] : [-0.2, -0.85, 0];
